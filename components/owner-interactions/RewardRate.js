...
function RewardRate({ currRewardRate, setCurrRewardRate }) {
  ...
  const rewardRateHandler = async () => {
    try {
      ...
      onNotify(
        true,
        `Reward Rate Changed to ${rewardRate} tokens per 1000 tokens staked`
      );
    } catch (err) {
  ...
  return (
    <div className="flex flex-col justify-content items-center">
      <div className="mx-5 font-bold">
        {" "}
        Current Reward Rate : {currRewardRate}{" "}
      </div>
      <div className="text-sm text-gray-500">
        Reward Rate is no. of reward tokens per 1000 tokens staked for each
        reward period
      </div>
