...
function RewardPeriod({ currRewardPeriod, setCurrRewardPeriod }) {
  const [rewardPeriod, updateRewardPeriod, resetRewardPeriod] =
    useInputState("");
  ...
  const rewardPeriodHandler = async () => {
    try {
      ...
      onNotify(
        true,
        `Reward Period Changed to ${rewardPeriod} seconds to update reward`
      );
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="flex flex-col justify-content items-center">
      <div className="mx-5 font-bold">
        {" "}
        Current Reward Period : {currRewardPeriod} seconds
      </div>
      <div className="text-sm text-gray-500">
        Reward Period is time in seconds in which you will get reward after each
        period
      </div>
      <div className="flex flex-row justify-content items-center">
        <form className="bg-white p-6 rounded-lg  text-center ">
          <label
            htmlFor="rewardPeriod"
