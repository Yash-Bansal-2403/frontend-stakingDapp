...
function OwnerInteraction() {
  ...
  useEffect(() => {
    const setValue = async () => {
      ...
      const pauseWithdrawalFromCall =
        await stakingContract.getPauseWithdrawalOfBalanceAndReward();
  ...
  return (
    <div className="flex flex-col justify-center">
      <RewardRate
        currRewardRate={currRewardRate}
        setCurrRewardRate={setCurrRewardRate}
      />
      <hr />
      <RewardPeriod
        currRewardPeriod={currRewardPeriod}
        setCurrRewardPeriod={setCurrRewardPeriod}
      />
      {/** */}{" "}
      <PauseWithdrawal
        pauseWithdrawal={pauseWithdrawal}
        setPauseWithdrawal={setPauseWithdrawal}
      />
    </div>
  );
}
